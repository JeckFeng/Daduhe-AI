...
def db_conn():
    ...
    conn = psycopg2.connect(
        ...
        user="daduhe", password="gis31415", dbname="mydatabase",
    )
...
def test_keyword_search_returns_chunks_containing_query(test_client):
    ...
    r = test_client.post("/api/v1/search", json={
        "query": "裂缝",
        "mode": "keyword",
    })
...
REQUIRED_CHUNK_METADATA = {
    "doc_id", "doc_type", "title", "section_number",
    "section_title", "page_number", "download_url",
}
...
def test_fuzzy_typo_tolerance(test_client):
    ...
    # Correct query
    r_correct = test_client.post("/api/v1/search", json={
        "query": "裂缝处理",
        "mode": "fuzzy",
    })
    ...
    # Typo query
    r_typo = test_client.post("/api/v1/search", json={
        "query": "裂逢处理",
        "mode": "fuzzy",
    })
    ...
    assert len(overlap) > 0, f"typo '{'裂逢处理'}' should overlap with correct '{'裂缝处理'}' results"
...
def index_test_client():
    ...
    s = Settings()
    mc = MilvusClient(uri=s.milvus_uri, user=s.milvus_user,
                      password=s.milvus_password, db_name=s.milvus_db)
...
def test_search_index_returns_202(index_test_client):
    ...
    r = index_test_client.post("/api/v1/search/index", json={
        "doc_id": "seed-doc-001",
...
def test_search_index_idempotent(index_test_client):
    ...
    # First call
    r1 = index_test_client.post("/api/v1/search/index", json={
        "doc_id": "seed-doc-001",
        "event_id": "evt-idem-1",
    })
    ...
    mc = MilvusClient(uri=s.milvus_uri, user=s.milvus_user,
                      password=s.milvus_password, db_name=s.milvus_db)
    mc.load_collection(INDEX_TEST_COLLECTION)
    first = mc.query(INDEX_TEST_COLLECTION,
                     filter=f'doc_id == "seed-doc-001"',
                     output_fields=["id"])

    # Second call
    r2 = index_test_client.post("/api/v1/search/index", json={
        "doc_id": "seed-doc-001",
        "event_id": "evt-idem-2",
    })
    ...
    mc.load_collection(INDEX_TEST_COLLECTION)
    second = mc.query(INDEX_TEST_COLLECTION,
                      filter='doc_id == "seed-doc-001"',
                      output_fields=["id"])
